...
def line_to_num_list_negs(string_in,base=10):
    out = []
    curnum = ""
    for ch in string_in.lower():
        if ch == '-' and curnum == "":
        ...
        if ord(ch) >= ord('0') and ord(ch) <= ord('9'):
            ...
            if len(curnum) > 0 and curnum != "-":
                out.append(int(curnum,base=base))
            curnum = ""
    if len(curnum) > 0 and curnum != "-": # handle the case where the line ends while parsing a number
        out.append(int(curnum,base=base))
    return out
