...
def search_from(st, end, starting_step):
    done = False
    latest_step = starting_step
    moves = [(st,starting_step,[])]
    ...
    while not done:
        ...
        if step_num > latest_step:
            ...
            if step_num % 20 == 0:
                best_move = moves[0]
                print(f"*A* move @ {best_move[1]} steps, time = {datetime.now()-st}")
        ...
        for pm in pos_moves:
            ...
            if pm[0] == end[0] and pm[1] == end[1]:
                print(f"Solution found: {pm}\nSteps: {step_num+1}\nPath: {path}")
                done = True 
                return pm,step_num+1

mv1,stpnum = search_from(exped_start, exped_end, 0)
print(f"=============== GOING BACK TO START (starting at step {stpnum}) ==========")
mv2,stpnum = search_from(exped_end, exped_start, stpnum)
print(f"=============== GOING BACK TO END (starting at step {stpnum}) ==========")
mv3,stpnum = search_from(exped_start, exped_end, stpnum)
